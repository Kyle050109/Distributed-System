...
PREPARE
Proposer enter Phase 1，send 'n' to all acceptors 
Log example: SEND PREPARE inst=ELECTION-1 n=... to=Mk

PROMISE
Acceptor receives PREPARE(n) and when n >= promisedN then return, including acceptor previous received highest (accN, accV) (if exists).
For: Proposer received most PROMISE, then enter Phase 2; if received accV is not null，must use highest accN corresponding accV.

ACCEPT
Proposer after getting PROMISE, choose 'v', if have accV then use accV, otherwise use itself's value, sendACCEPT(n, v)too all acceptors.

ACCEPTED
Acceptor accepted ACCEPT(n, v), condition is n >= promisedN. It can record acceptedN=n, acceptedV=v and reply ACCEPTED.
For: Proposer after observed most ACCEPTED then make decision.

NACK
Acceptor reject PREPARE or ACCEPT when n < promisedN, bring a higher promisedN/
For: Proposer needs to increase proposal number after retrying.

DECIDE
When proposer observed most ACCEPTED, decide in local and tell others DECIDE(v)


Human-readable markers:
CONSENSUS:
Learner print the result:
CONSENSUS: <Mx> has been elected Council President!

When there is already a winner:
Already decided …; ignoring propose …

When the process starts, it prints the persistent state that has been recovered from the disk:
